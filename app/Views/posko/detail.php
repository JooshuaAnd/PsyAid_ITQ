...
<div class="card card-custom bg-white p-4 mb-4 shadow-sm">
    <div class="row align-items-center g-3">
        <div class="col-12 col-lg-8">
            <a href="<?= site_url('/command-center') ?>" class="btn btn-outline-secondary btn-sm mb-2">
                <i class="bi bi-arrow-left me-1"></i> Kembali ke Command Center
            </a>
            <h3 class="fw-bold text-dark mb-1">
                <i class="bi bi-geo-alt-fill text-danger me-2"></i> <?= esc($posko['name']) ?>
            </h3>
            <p class="text-muted small mb-2">
                <i class="bi bi-building me-1"></i> <?= esc($posko['regency_name']) ?>, <?= esc($posko['province_name']) ?> 
            ...
            <div class="d-flex align-items-center flex-wrap gap-2 mt-3 pt-2 border-top">
                <span class="small text-muted fw-semibold me-1"><i class="bi bi-people-fill me-1"></i> Personel Bertugas:</span>
        ...
        <div class="col-12 col-lg-4 text-lg-end">
            <a href="<?= site_url('/victim/create/' . $posko['id']) ?>" class="btn btn-success fw-bold px-3 py-2 me-2 shadow-sm">
            ...
            <?php elseif ($posko['status'] === 'recovery'): ?>
                <span class="badge bg-warning text-dark fs-6 px-3 py-2"><i class="bi bi-arrow-repeat me-1"></i> Masa Recovery</span>
            <?php else: ?>
...
<div class="row g-3 mb-4">
    ...
    <div class="col-12 col-sm-6 col-lg-3">
        <div class="card card-custom bg-white p-3 border-start border-4 border-warning h-100">
            ...
            <div class="mt-2">
                <div class="d-flex justify-content-between small mb-1">
                    <span class="text-dark fw-semibold"><i class="bi bi-dash-circle text-warning me-1"></i> Medium Risk</span>
                    <strong><?= esc($stats['risk_medium']) ?></strong>
                </div>
                <div class="d-flex justify-content-between small">
                    <span class="text-info fw-semibold"><i class="bi bi-check-circle text-info me-1"></i> Low Risk</span>
...
<div class="card card-custom bg-white p-4 shadow-sm mb-4">
    ...
    <form method="GET" action="<?= site_url('/posko/' . $posko['id']) ?>" class="row g-2 mb-3 bg-light p-3 rounded border">
        <div class="col-12 col-md-5">
            <input type="text" name="q" class="form-control form-control-sm" 
                   placeholder="Cari Nama Penyintas atau NIK..." 
        ...
        <div class="col-12 col-sm-6 col-md-3">
            <select name="screening_status" class="form-select form-select-sm">
                ...
                <option value="sudah" <?= ($searchFilters['screening_status'] ?? '') === 'sudah' ? 'selected' : '' ?>>Sudah Skrining</option>
                <option value="belum" <?= ($searchFilters['screening_status'] ?? '') === 'belum' ? 'selected' : '' ?>>Belum Skrining</option>
            </select>
        </div>
        <div class="col-12 col-sm-6 col-md-3">
            <select name="risk_level" class="form-select form-select-sm">
                <option value="">-- Semua Risk Level --</option>
                <option value="high" <?= ($searchFilters['risk_level'] ?? '') === 'high' ? 'selected' : '' ?>>High Risk</option>
                <option value="medium" <?= ($searchFilters['risk_level'] ?? '') === 'medium' ? 'selected' : '' ?>>Medium Risk</option>
                <option value="low" <?= ($searchFilters['risk_level'] ?? '') === 'low' ? 'selected' : '' ?>>Low Risk</option>
            </select>
        </div>
        <div class="col-12 col-md-1 d-flex gap-1">
            <button type="submit" class="btn btn-sm btn-primary w-100"><i class="bi bi-search"></i></button>
            <a href="<?= site_url('/posko/' . $posko['id']) ?>" class="btn btn-sm btn-outline-secondary" title="Reset"><i class="bi bi-x-circle"></i></a>
        </div>
    </form>
    ...
    <div class="table-responsive">
        <table class="table table-hover align-middle mb-0">
            ...
            <tbody>
                ...
                <?php else: ?>
                    <?php $no = 1; foreach ($victims as $v): ?>
                        <tr class="<?= $v['risk_level'] === 'high' ? 'table-danger bg-opacity-10' : '' ?>">
                            <td class="fw-bold text-muted"><?= $no++ ?></td>
                            <td>
                                <a href="<?= site_url('/victim/detail/' . $v['id']) ?>" class="fw-bold text-dark text-decoration-none hover-danger">
                                    <?= esc($v['nama']) ?>
                                </a>
                            ...
                            <td>
                                <?php if ($v['screening_id']): ?>
                                    ...
                                    <div class="fs-8 text-muted">Distress: <strong><?= esc($v['skala_distress']) ?>/10</strong></div>
                                <?php else: ?>
                            ...
                            <td class="text-end">
                                <a href="<?= site_url('/victim/detail/' . $v['id']) ?>" class="btn btn-sm btn-outline-danger px-2 py-1" title="Buka Detail Penyintas">
